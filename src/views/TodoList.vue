...
<template>
  <div class="max-w-4xl mx-auto space-y-6 animate-fade-in">
    <div class="flex justify-between items-end">
      <div>
        <h2 class="text-2xl font-bold text-slate-800 dark:text-white transition-colors duration-300">待办事项</h2>
        <p class="text-slate-500 dark:text-gray-400 transition-colors duration-300">高效管理你的学习与生活</p>
      </div>
      <div class="text-sm font-medium text-slate-600 dark:text-gray-300 bg-white dark:bg-gray-800 px-3 py-1 rounded-full shadow-sm border dark:border-gray-700 transition-all duration-300">
        已完成: {{ todoStore.todos.filter(t => t.completed).length }} / {{ todoStore.todos.length }}
      </div>
    </div>
