...
const menuItems = computed(() => {
  const text = languageStore.tm('sidebar');
  return [
    { path: '/', label: text.home, icon: LayoutDashboard },
    { path: '/todo', label: text.todo, icon: ListTodo },
    { path: '/about', label: text.about, icon: Info },
    { path: '/chat', label: text.chat, icon: Bot },
  ];
});

const labels = computed(() => languageStore.tm('sidebar'));
...
<template>
  <div class="w-72 h-screen bg-white/80 dark:bg-gray-900/95 backdrop-blur-md border-r border-slate-200 dark:border-gray-800 flex flex-col z-20 transition-all duration-300 ease-in-out">
    ...
    <nav class="flex-1 px-4 space-y-1.5">
      <p class="px-4 text-xs font-bold text-slate-400 dark:text-slate-600 uppercase tracking-wider mb-4 mt-2">{{ labels.navigation }}</p>
